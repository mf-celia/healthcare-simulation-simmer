      ...
      trajectory("regular_path") %>%
        ...
        branch(
          ...
          trajectory("wait_retry") %>%
            set_attribute("retries", 1, mod = "+") %>%  # Incrementa en +1
...
resources <- get_mon_resources(env) |> 
  mutate(center_id = as.numeric(str_extract(resource, "^\\d+")),
...
patient_attributes <- get_mon_attributes(env) |> 
...
patient_arrivals <- get_mon_arrivals(env) |> 
...
attributes_summary <- patient_attributes %>%
  mutate(
...
```{r}
retries_summary <- get_mon_attributes(env) %>%
  filter(key == "retries") %>%
  mutate(id = as.numeric(str_remove(name, "^p_")),
         id = id/10) |> # eliminamos el 0 que sobra al final
  left_join(llegadas |> 
              dplyr::select(id, area, center_id),
            by = c("id")) |> 
  group_by(area, center_id, id) %>%
  summarise(days_to_attend = max(value) + 1)
```
